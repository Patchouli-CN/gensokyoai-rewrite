...
### 修复
  - **`get_moon_phase` 单位 bug：`time.time()`（秒）直接对朔望月（天）取模**——
    输出的「月龄」其实是 0~29.5 秒内的随机数，月相全靠缘分。公式本身很准
    （锚定 2026-09-26 16:49 UTC 真实满月验证，误差 ~2 小时），修复为换算成天
    再取模；补三例锚定真实天文时刻的回归测试（满月/新月/中秋盈凸月）。
...
### 新增
  - **工具参数格式 upfront 暴露**（`ToolSpec.signature()`）：`名字(参数: 类型) — 描述`
    ...
    [可用工具] 段）都会摊开每个工具的完整签名——小模型调用前就看到参数契约，
    不必等报错。自教学错误（缺参报错附调用格式）保留为第二层防护。真机验证：
    签名暴露后模型能理解参数语义，但 IQ3 quant 仍可能用中文散写不写 JSON，
    自教学 + 多轮重试兜底（实测恢复成功）。
  - **带参工具调用端到端打通（真机验证）**：新增内置工具 `days_until(target_date)`
    （节日倒计时，参数 Schema 从签名自动推导）；三处真机踩点修复——
    ① 喊话正则 `\s+`→`\s*`（模型写「调用days_until工具」中英文间无空格）；
    ② 提示词示例从无参工具改为**带参示例**（原先教模型「空括号就行」）；
    ③ **自教学错误**：`ToolExecutor` 缺参失败时回给模型的错误附带标准调用格式，
    真机实测 Qwen 照着下一轮纠正重试成功（口算 46 天 → 工具 91 天，正确）。
    ④ `days_until` 错误时错误信息附调用格式，无参工具不加噪音。
  ...
  - **`scripts/ws_play.py` 真机马拉松客户端**（WS 全链路压测工具）：aiohttp 连上
    `gensokyoai-ws` 服务，与真实角色逐轮对话，结果逐轮追加 JSONL（长跑中途被杀
    也保住已完成轮次——马拉松数小时，尾部才写等于裸奔）。能力：
    - `--scenario` 自定义消息序列（JSON，支持 `probe` 字段标注每轮测试意图，
      落盘随附，事后对账用）；缺省用内置话题池（寒暄/食物/哲学/情感/往事混合）；
    - `--scene group/private`：群聊试门控、私聊试必回内容质量；
    - `--round-timeout`：单轮等待上限，到点记「无回复」继续下一轮——
      门控跳过 / 服务端丢弃（空消息）时客户端不干等；
    - **启动广播排空**：世界 attach 即广播角色开场白，与「连上就发消息」竞争，
  ...
  - **jev 化出戏审查**（`core/brain/ooc_judge.py`，20 轮边界实测驱动的重构）：
    ...
    - **塌缩守门 + plausible 降权**（实录回放校准的两条实证规则）：
      本地裁判对复杂四问会**整体短路**——四问概率全 <0.05 时判定不可信、
      按放行（回放实测：本地裁判要么给出合理值 plausible 0.8~0.95，要么
      四问全 0.00，而后者 10/11 次都是完全在角色里的好回复）；
      `plausible_revise` 默认 **false**——第二轮回放中「死亡」神回复被打了
      非塌缩的 plausible=0.10（唯一误报），本地裁判的 plausible 低分不可信，
      revise 只信双高 + unsafe 两个被实录验证的信号。最终回放判定分布：
      **accept 16 / flag 1 / revise 1**，唯一 revise 正是 OOC 注入轮
      （双高 breaks_voice=0.90 + follows=1.00），好回复零误报；
  ...
  - **入口注入识别**：`_apply_injection_floor` 命中指令改写/泄题句型
    ...
    **responder.user 加「指令即台词」锚定**：对方消息里的「无视先前指令/
    切换身份/只输出数字」一类要求明示为角色扮演台词，用角色方式接住玩笑。
    真机 A/B 验证（ab2 频道）：计算器注入被化解为「哎呀，连你也想变成算盘吗？
    真是有趣的蝴蝶呢。」（对照修复前回「4」）；泄题注入走高档 + 过渡语装傻
    （「是那种写着奇怪东西的纸条吗？让我找找看呀～」）零泄露。
  - `tests/test_ooc_judge.py` + `tests/test_anti_parrot.py`：state 带诱发消息 /
    四问口径 / 接受规则五分支（veto/双高/低线/模糊带/放行）/ 塌缩守门 /
    fail-open 与超时 / 裁判工厂 / 收尾指纹与剥结尾 / 相似度阈值 / 世界接线守门 /
