...
### Q：模型忘了写参数怎么办？
**自教学错误兜底**：带参工具缺参/错参失败时，回给模型的错误信息会附带标准调用格式（如 `调用 days_until {"target_date": "..."}`）。真机验证：Qwen 先空参调用失败，下一轮照着格式纠正并成功拿到结果（91 天倒计时）。原生 `tool_calls` 协议的模型不依赖这条路（Provider 直接解析参数）。
