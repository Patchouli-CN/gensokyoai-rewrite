...
- **可定制思考链流水（ThinkPipeline）**：角色卡 `think_chain` 按名引用思考步骤（提示词集中注册在 `prompts/manager.py`），**换链 = 换角色的思维方式，零代码**；也支持**内联自定义步骤**（卡片里直接写 `name`/`instructions`，作者零代码造步骤）。步骤用 `ThinkPipeline("标签") >> ThinkStep(...)` 拼装（泛型链式基类 `FluentAPI[T]`），步骤间以压缩 digest 交接结论。
