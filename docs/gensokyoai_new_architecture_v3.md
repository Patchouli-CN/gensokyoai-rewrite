...
| 子模块 | 职责 |
|--------|------|
| 推理链引擎 | 管理 NONE / LOW / MID / HIGH / MAX 五档推理深度 |
| OOC Verdict | 检测回复是否偏离角色人设 |
| Knowledge Base | 知识检索，为推理提供资料支撑 |
| 发言门控（Gate） | 反应路径「该不该接话」：规则预筛明显信号，群聊模糊带交裁判，故障退化为规则兜底（见下文） |

**发言门控（JeV 式混合门控，`core/brain/gate.py` + `core/brain/judge.py`）：**
移植 Mist-wu/qqbot 的 jev 机制。收尾语 / 私聊 / 被 @ 等明显信号由零成本规则直判，
只有「群里没人点名的新消息」才调裁判（`Judge` 协议）：本地模型（`LocalJudge`，
主模型无状态小调用）或真 jev（`TypeSafeJudge`，官方 SDK，可选依赖）返回
`should_reply` / `addressed` / `needs_search` 三个 0~1 概率，`should_reply` 达到
`gate.group_threshold`（默认 0.6）才发言。裁判输入含 bot 近窗口活跃度
（`PresenceTracker`），防刷屏靠模型自觉、不设硬冷却；裁判异常时退化为「点名才回」。
门控跳过的消息仍会写入记忆（不回复≠没听过）。`gate.enabled=false` 时维持
「每条都回」的旧行为。
...
| 形态 | 机制 | 适用 |
|------|------|------|
| 接力思考（`engine.py::_relay_think`） | 模型每轮自述 `need_continue_think`，按档位设轮数上限，工具调用不计轮数地穿插其中 | 默认路径；追求深度与灵活 |
| 思考链流水（`pipeline.py::ThinkPipeline`） | 角色卡 `think_chain` 定好步骤序列，逐步无状态小调用 + digest 交接，收尾步映射 `BrainConclusion` | 定制「思考方向」：换链=换思维方式，零代码 |

思考链步骤是数据（`ThinkStep`：指令/预算/温度/超时/optional），链用 `utils/fluent.py::FluentAPI`
的 `>>` 拼装（同步不可变，支持链合并）；执行是 async：步骤**串行 await**（N+1 吃 N 的
digest，单模型串行无并发收益）、每步 `wait_for` 超时隔离、只捕 `Exception` 放行
`CancelledError`、零 fire-and-forget。整链失败降级快速路径，拖死不了回合。

**五档动态深度（`BrainThinkEffort`，档位 = 思考链跑多深 / 接力跑几轮）：**

| 档位 | 思考链（配了卡） | 接力思考（未配卡） | 适用场景 |
|------|----------------|------------------|---------|
| NONE | 不跑链（关键词意图，零模型调用） | 不跑 | 空输入 / 纯寒暄收尾 |
| LOW | 只留**收尾步**：不铺垫，直出结论（1 次调用） | 2 轮 | 日常寒暄、简单问答 |
| MID | **首 + 尾**：一点感知 + 决策（2 次调用） | 3 轮 | 一般对话，需看角色情绪 |
| HIGH | **全链**（N 次调用） | 5 轮 | 复杂剧情推进、多角色交互 |
| MAX | 全链 + 每步**深思考**（预算 ×2、低温） | 8 轮 | 情感转折、重大剧情节点 |

档位由 `route()` 规则路由（长度/情绪词/剧情词/参与者，零模型调用），并可由
HealthCenter 的 OOC 干预整体抬高下限（`_effort_floor`）。**同一输入、不同角色卡 =
不同思考链；同一角色、不同复杂度 = 不同深度**——算力花在刀刃上。

**路由模型化（`gate.route_by_model`）：「该不该过脑子」本身就是最典型的
System-1 决策，已下沉给裁判**——门控那次裁判调用同时回答 `should_reply`
（该不该说）与 `needs_deep`（想多深，经 `deep_cuts` 切点映射档位）；规则
`route()` 退居兜底（裁判不可用/缺答时回落）。真机探针证实本地模型比规则
路由更能看懂情绪与关系信号（38 字情绪倾诉：模型 mid，规则只会判 low）。
